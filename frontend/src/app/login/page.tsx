...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim() || !passcode.trim()) return;

    setLoading(true);
    setError(null);

    try {
      const res = await apiClient<{ user: { role: string } }>("/auth/login", {
        method: "POST",
        json: { username: username.trim(), passcode: passcode.trim() }
      });
      ...
      setError(err.message || t("loginBack")); // fallback or exact error
    } finally {
      setLoading(false);
    }
  };
